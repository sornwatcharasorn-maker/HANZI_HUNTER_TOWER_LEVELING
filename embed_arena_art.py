...
def put_hero(src, frames):
    if not frames:
        ...
        rows = ",\n".join("      { k: '%s', d: '%s' }" % (k, d) for _, k, d in frames)
        new = '    const BA_HERO = [\n' + rows + '\n    ];'
    out, n = re.subn(r'    const BA_HERO = \[.*?\];', lambda m: new, src, count=1, flags=re.S)
    if n != 1:
        sys.exit('เขียน BA_HERO ไม่สำเร็จ — หา anchor ไม่เจอ')
    return out
...
def cur_hero(src):
    m = re.search(r'    const BA_HERO = \[(.*?)\];', src, re.S)
    if not m:
        return []
    return [(k, d) for k, d in re.findall(r"k:\s*'(\w+)'\s*,\s*d:\s*'([^']*)'", m.group(1))]
...
def main():
    ...
    # ── เตรียมเฟรมฮีโร่ ────────────────────────────────────────────────────
    do_hero = (not want) or ('hero' in want)
    hero_raw = []
    if do_hero:
        sheets = {}
